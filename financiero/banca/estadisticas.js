...
router.get("/", (req, res) => {
  // 1. Capturamos los datos originales
  ...
  // 2. Creamos las variables seguras
  const safeAceptados = escape(aceptadosRaw);
  const safeRechazados = escape(rechazadosRaw);
  const safeAgendados = escape(agendadosRaw);
  const safePendientes = escape(pendientesRaw);

  res.setHeader("Content-Type", "image/svg+xml");
  // 3. Usamos las variables seguras en el SVG
  res.send(`
    <svg xmlns="http://www.w3.org/2000/svg" width="375" height="298" fill="none">
      ...
      <rect width="171" height="112.5" x="10.75" y="30.75" fill="#314150" rx="11.25"/>
      ...
      <text x="30" y="78" font-size="42" font="Lato" font-weight="bolder" fill="#5ABE45">${safeAceptados}</text>
      <text x="47" y="120" font-size="18" font-weight="lighter" fill="#fff">Aceptados</text>
      <rect width="171" height="112.5" x="193.25" y="30.75" fill="#314150" rx="11.25"/>
      ...
      <text x="212" y="80" font-size="42" font-weight="bolder" fill="#FF575A">${safeRechazados}</text>
      <text x="229" y="120" font-size="18" font-weight="lighter" fill="#fff">Rechazados</text>
      <rect width="171" height="112.5" x="10.75" y="154.75" fill="#314150" rx="11.25"/>
      ...
      <text x="30" y="200" font-size="42" font-weight="bolder" fill="#2ABEDF">${safeAgendados}</text>
      <text x="47" y="244" font-size="18" font-weight="lighter" fill="#fff">Agendados</text>
      <rect width="171" height="112.5" x="193.25" y="154.75" fill="#314150" rx="11.25"/>
      ...
      <text x="212" y="200" font-size="42" font-weight="bolder" fill="#FFB109">${safePendientes}</text>
